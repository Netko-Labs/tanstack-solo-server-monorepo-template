...
import type { ChatMessage, Member, RoomEvent } from '@temp-repo/realtime-domain'
...
/**
 * In-memory room membership + fan-out. Each room has an EventEmitter that the
 * tRPC subscription (observable) listens to; presence is keyed by userId
 * (ephemeral), chat is persisted separately via the repository.
 */
class RoomHub {
  ...
  /** Listen to a room's events; returns an unsubscribe fn. */
